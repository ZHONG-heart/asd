<template>
  <div class="app-container">
    ...
    <el-dialog :title="this.postTitle+'-贴子详情'" :visible.sync="dialogVisible" width="30%">
      ...
      <!-- 图表 -->
      <div v-loading="listLoadingTrue" >
         <el-table :key='tableKey' :data="detailList" element-loading-text="努力加载中..." border fit highlight-current-row style="width: 100%;">
          <el-table-column align="center" label="用户">
            <template scope="scope">
              <span style="margin-left:10px;cursor:pointer" v-html="scope.row.Userid" v-clipboard:copy='scope.row.Userid' v-clipboard:success='clipboardSuccess'>{{scope.row.Userid}}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" label="用户名">
            <template scope="scope">
              <span style="margin-left:10px;cursor:pointer" v-html="scope.row.author" v-clipboard:copy='scope.row.author' v-clipboard:success='clipboardSuccess'>{{scope.row.author}}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    showPostDetail(row) {
      ...
      this.getCommunityPostsDetail(row.Threadid, row.authorid);
      this.temporaryThread = row.Threadid;
